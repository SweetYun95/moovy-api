...
app.use((err, _req, res, _next) => {
   ...
   const isSequelizeError = typeof err?.name === 'string' && err.name.startsWith('Sequelize')
   const message = (isSequelizeError ? err?.parent?.sqlMessage || err?.original?.sqlMessage : null) || err.message || '서버 내부 오류'
   ...
   // details는 필요할 때만(그리고 과도한 중복/노이즈 방지)
   const details = isSequelizeError ? undefined : err.details
